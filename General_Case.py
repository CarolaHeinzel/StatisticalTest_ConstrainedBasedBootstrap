...
def generate_reduced_simplex(K, step, mass, total=1.0, tol=1e-9):
    result = []

    def is_on_grid(x):
        return abs(x / step - round(x / step)) < tol
    ...
    def recurse(current, depth, remaining):
        if depth == K - 1:
            ...
            if val < -tol:
                return
            if val <= mass + tol and is_on_grid(val):
                candidate = current + [max(0.0, val)]
                if abs(sum(candidate) - total) < 1e-8 and abs(max(candidate) - mass) < 1e-8:
                    result.append(candidate)
            return

        max_val = min(mass, remaining)
        n_steps = int(round(max_val / step))

        for i in range(n_steps + 1):   # allow zero
            val = round(i * step, 10)
...
for M in [50, 100, 200, 500, 1000]:
    for T in [0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]: 
        for K in [2, 5]:
            for epsilon in [0.85]:
    ...
    df_0_swap = pd.DataFrame(results_0)
    df_0_swap.to_csv("results_0_swap_K2_5_1804_all_equal.csv", index=False)
